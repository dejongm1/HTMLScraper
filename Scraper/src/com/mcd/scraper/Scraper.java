		...
		if (args.length == 0) {
			...
			try {
				...
				if (Boolean.valueOf(args[2])) {
					...
					doc = Jsoup.parse(input, "UTF-8", args[0]);
	
				} else {
					doc = Jsoup.connect(args[0]).userAgent("Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/59.0.3071.104 Safari/537.36").get();
				}
				Elements tags = doc.select(args[1]);
